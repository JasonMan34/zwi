...
export class MinesweeperTile {
  ...
  public getAdjacent(...statuses: TileStatus[]) {
    ...
    for (let row = rowStart; row <= rowEnd; row++) {
      for (let col = colStart; col <= colEnd; col++) {
        ...
        if (tile !== this && (statuses.length === 0 || statuses.includes(tile.status))) {
          adjacent.push(tile);
        }
      }
    }
  ...
  public updateIsFinal() {
    this.isFinal = this.status !== 'hidden' && this.getAdjacent('hidden').length === 0;
  }
  ...
  public click() {
    ...
    if ((this.value === 0 && !this.isMine) || (wasRevealed && this.flagCount === this.value)) {
